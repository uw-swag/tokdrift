...
class Evaluator:
    def __init__(self, accelerator, model, tokenizer, logs_dir, args, config):
    ...
    def generate_text(self, task_name, intermediate_generations=None):
        ...
        # hidden_states_save_path = f"{os.path.splitext(self.args.save_generations_path)[0]}_{task_name}"
        hidden_states_save_path = self.args.hidden_states_save_path
        hidden_states_difference_save_path = self.args.hidden_states_difference_save_path
    ...
    def evaluate(self, task_name, intermediate_generations=None):
        ...
        if self.accelerator.is_main_process:
            ...
            results = task.process_results(generations, references, self.logs_dir)
            return results
