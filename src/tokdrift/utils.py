...
def complete_code(
    ...
    hidden_states_save_path: Optional[str] = None,
    hidden_states_difference_save_path: Optional[str] = None,
    config=None,
    **gen_kwargs,
):
    ...
    # Dictionary to store hidden states for Token 0, all layers, for each sample
    if hidden_states_difference_save_path is not None:
        print("Skipping saving hidden states")
    else:
        print(f"Saving hidden states to {hidden_states_save_path}")
    
    hidden_states_dict = defaultdict(list) if hidden_states_save_path else None
    for step, batch in tqdm(
        ...
        with torch.no_grad():
            ...
            if "ids_encoder" in batch:
                if is_wrapped:
                    model_output = accelerator.unwrap_model(model).generate(
            ...
            # Extract hidden states for Token 0, all layers, if required
            if hidden_states_dict is not None and hasattr(model_output, 'hidden_states') and model_output.hidden_states is not None:
                if len(model_output.hidden_states) > 0:  # Ensure we have at least Token 0
                    token_0_hidden = model_output.hidden_states[0]  # Get Token 0 hidden states across all layers
                    
                    # Extract hidden states for each task in this batch
                    batch_task_ids = batch["task_id"]
                    for i, task_id in enumerate(batch_task_ids):
                        # Keep tensors and store them directly
                        token_0_all_layers = []
                        for layer_idx in range(len(token_0_hidden)):
                            # Shape: [batch_size, seq_len, hidden_size] -> [seq_len, hidden_size] for this task
                            layer_hidden = token_0_hidden[layer_idx][i].cpu()  # Remove batch dimension, keep as tensor
                            token_0_all_layers.append(layer_hidden)
                        
                        # Stack all layers into a single tensor: [num_layers, seq_len, hidden_size]
                        stacked_hidden_states = torch.stack(token_0_all_layers, dim=0)
                        hidden_states_dict[task_id.item()].append(stacked_hidden_states)
        

            generated_tokens = model_output.sequences
    ...
    # Calculate hidden states difference if required
    output_hidden_states = []
    if hidden_states_save_path is not None and hidden_states_difference_save_path is not None:
        if hidden_states_dict is not None:
            if accelerator.is_main_process:
                for task_id, hidden_states_list in hidden_states_dict.items():
                    if hidden_states_list:
                        output_hidden_states.append(hidden_states_list[0])
        # Set hidden_states_dict to None to avoid saving hidden states
        hidden_states_dict = None

    if len(output_hidden_states) > 0 and config is not None:
        vector_processor = VectorProcessor(config, hidden_states_save_path=hidden_states_save_path, hidden_states_difference_save_path=hidden_states_difference_save_path)
        vector_processor.process_hidden_states(output_hidden_states)
    elif len(output_hidden_states) > 0:
        print("Config not provided - skipping vector processing")
    else:
        print("No hidden states to process")

        
    # Save hidden states if required
    if hidden_states_dict is not None and hidden_states_save_path:
        if accelerator.is_main_process:
            # Extract directory name from the path (use filename as folder name)
            base_name = os.path.basename(hidden_states_save_path)
            save_dir = os.path.join(os.path.dirname(hidden_states_save_path), base_name)
            ...
            for task_id, hidden_states_list in hidden_states_dict.items():
                if hidden_states_list:  # Check if we have hidden states for this sample
                    # If multiple hidden states per sample (multiple generations), take the first one
                    # Shape: [num_layers, seq_len, hidden_size]
                    sample_hidden_states = hidden_states_list[0]
                    
                    sample_file_path = os.path.join(save_dir, f"sample_{task_id}.pt")
                    torch.save(sample_hidden_states, sample_file_path)
            
            print(f"Hidden states saved to directory: {save_dir}")
            print(f"Saved {len(hidden_states_dict)} sample files as .pt format")
