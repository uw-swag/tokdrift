...
SOURCE_LANGUAGES = ["python", "java", "cpp"]
...
def create_all_tasks():
    """Create all CodeNet translation tasks"""
    translate = {f"codenettranslate-{source}2{target}": create_task("translate", source, target) for source in SOURCE_LANGUAGES for target in TARGET_LANGUAGES}
    return {**translate}
...
class Codenet(Task):
    """Base class for Codenet tasks"""

    DATASET_PATH = "./datasets/codenet"
    DATASET_NAME = None
...
class CodenetGenerative(Codenet):
    """Base class for Codenet generative tasks"""
    ...
    def process_evaluation_logs(self, baseline_logs_path, variant_logs_path, task_idx, variant_ids):
        ...
        for i, idx in enumerate(task_idx):
            ...
            if baseline_log['passed'] and not variant_log['passed']:
                result['new_errors'] = variant_log.get('errors', variant_log.get('error', []))
            elif not baseline_log['passed'] and variant_log['passed']:
                result['baseline_errors'] = baseline_log.get('errors', baseline_log.get('error', []))
...
class CodenetTranslationBase(CodenetGenerative):
    """Base class for Codenet translation tasks"""
    ...
    def postprocess_generation(self, generation, idx):
        ...
        gen_text = generation[len(prompt):].strip()
            ...
            # If no markdown blocks, try to extract until stop words
