...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save the baseline hidden states",
    )
    parser.add_argument(
        "--hidden_states_difference_save_path",
        type=str,
        default=None,
...
def main():
    ...
    logs_dir = os.path.dirname(args.save_generations_path)
    if not os.path.exists(logs_dir) and not args.generation_only:
        raise ValueError(f"Output directory {logs_dir} does not exist")

    if args.hidden_states_difference_save_path is not None:
        if args.hidden_states_save_path is None:
            raise ValueError("If --hidden_states_difference_save_path is not None, --hidden_states_save_path must be provided to calculate the difference between the baseline hidden states and the variant hidden states")
        if not os.path.exists(args.hidden_states_difference_save_path):
            raise ValueError(f"Output directory {args.hidden_states_difference_save_path} does not exist")
    ...
    if args.load_generations_path:
        ...
        evaluator = Evaluator(accelerator, None, None, logs_dir, args, config)
        for task in task_names:
            # Configure the config object for this specific task
            config.config_experiment(task, args.model)  # No model needed for evaluation-only mode
        ...
        evaluator = Evaluator(accelerator, model, tokenizer, logs_dir, args, config)
        ...
        for idx, task in enumerate(task_names):
            # Configure the config object for this specific task
            config.config_experiment(task, args.model)
